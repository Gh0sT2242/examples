...
DRIAdmin.Functions.DrawText3D = function(x, y, z, text, lines)
...
AddEventHandler('DRICore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("dri-admin:server:loadPermissions")
end)
...
PermissionLevels = {
	...
	[4] = {rank = "p2", label = "p2"},
	[5] = {rank = "p3", label = "p3"},
    [6] = {rank = "admin", label = "Admin"},
    [7] = {rank = "god", label = "God"},
}
...
RegisterNetEvent('dri-admin:client:openMenu')
AddEventHandler('dri-admin:client:openMenu', function(group, dealers)
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('admin', 'Avalanche Admin')
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('selfOptions') then
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                ...
                TriggerServerEvent("dri-admin:server:togglePlayerNoclip", targetId)
            end
            if WarMenu.Button('Revive') then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent('dri-admin:server:revivePlayer', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('serverMan') then
            ...
            if WarMenu.ComboBox('Server time', times, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("dri-weathersync:server:setTime", time.hour, time.minute)
            end
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            if WarMenu.MenuButton('Kill', currentPlayer) then
                TriggerServerEvent("dri-admin:server:killPlayer", GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Revive', currentPlayer) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent('dri-admin:server:revivePlayer', target)
            end
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("dri-admin:server:togglePlayerNoclip", target)
            end
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("dri-admin:server:Freeze", target, isFreeze)
            end
            ...
            if WarMenu.MenuButton("Give Clothing Menu", currentPlayer) then
                ...
                TriggerServerEvent('dri-admin:server:OpenSkinMenu', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('teleportOptions') then
            ...
            if WarMenu.MenuButton('Bring', currentPlayer) then
                ...
                TriggerServerEvent('dri-admin:server:bringTp', GetPlayerServerId(currentPlayer), plyCoords)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent('dri-admin:server:setPermissions', target, group)

                DRICore.Functions.Notify('You have '..GetPlayerName(currentPlayer)..'\'s group has changed to '..group.label)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban Length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("dri-admin:server:banPlayer", target, time, reason)
                end
            end
            if WarMenu.MenuButton('Kick', currentPlayer) then
                ...
                if reason ~= nil and reason ~= "" then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("dri-admin:server:kickPlayer", target, reason)
                end
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    TriggerServerEvent('dri-weathersync:server:setWeather', AvailableWeatherTypes[k].weather)
                    DRICore.Functions.Notify('Again has changed to: '..AvailableWeatherTypes[k].label)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened('createDealer') then
            if WarMenu.ComboBox('Min. Time', times, currentMinTimeIndex, selectedMinTimeIndex, function(currentIndex, selectedIndex)
                ...
                DRICore.Functions.Notify('Time confirmed!', 'success')
            end
    ...
    while true do
        ...
        if showNames then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(GetPlayerPed(-1)), 5.0)) do
                ...
                DRIAdmin.Functions.DrawText3D(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z + 1.0, '['..PlayerId..'] '..PlayerName)
            end
        else
            Citizen.Wait(1000)
        end
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                DRIAdmin.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Obj: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nPress [~g~E~s~] to delete this object!", 2)
...
RegisterNetEvent('dri-admin:client:bringTp')
AddEventHandler('dri-admin:client:bringTp', function(coords)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('dri-admin:client:Freeze')
AddEventHandler('dri-admin:client:Freeze', function(toggle)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('dri-admin:client:SendReport')
AddEventHandler('dri-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('dri-admin:server:SendReport', name, src, msg)
end)

RegisterNetEvent('dri-admin:client:SendStaffChat')
AddEventHandler('dri-admin:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('dri-admin:server:StaffChatMessage', name, msg)
end)

RegisterNetEvent('dri-admin:client:SaveCar')
AddEventHandler('dri-admin:client:SaveCar', function()
    local ped = GetPlayerPed(-1)
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = GetVehicleNumberPlateText(veh)
        local props = DRICore.Functions.GetVehicleProperties(veh)
        local hash = props.model
        if DRICore.Shared.VehicleModels[hash] ~= nil and next(DRICore.Shared.VehicleModels[hash]) ~= nil then
            TriggerServerEvent('dri-admin:server:SaveCar', props, DRICore.Shared.VehicleModels[hash], GetHashKey(veh), plate)
        else
            DRICore.Functions.Notify('You cannot put this vehicle in your garage..', 'error')
        end
    else
        DRICore.Functions.Notify('You are not in a vehicle..', 'error')
    end
end)
...
RegisterNetEvent('dri-admin:client:SetModel')
AddEventHandler('dri-admin:client:SetModel', function(skin)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('dri-admin:client:SetSpeed')
AddEventHandler('dri-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('dri-weapons:client:SetWeaponAmmoManual')
AddEventHandler('dri-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = GetPlayerPed(-1)
    if weapon ~= "current" then
        local weapon = weapon:upper()
        SetPedAmmo(ped, GetHashKey(weapon), ammo)
        DRICore.Functions.Notify('+'..ammo..' Ammo for the '..DRICore.Shared.Weapons[GetHashKey(weapon)]["label"], 'success')
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            DRICore.Functions.Notify('+'..ammo..' Ammo for the '..DRICore.Shared.Weapons[weapon]["label"], 'success')
        else
            DRICore.Functions.Notify('You dont have a weapon..', 'error')
        end
    end
end)

RegisterNetEvent('dri-admin:client:GiveNuiFocus')
AddEventHandler('dri-admin:client:GiveNuiFocus', function(focus, mouse)
    SetNuiFocus(focus, mouse)
end)

RegisterNetEvent('dri-admin:client:GainStress')
AddEventHandler('dri-admin:client:GainStress', function()
    TriggerServerEvent('dri-hud:Server:GainStress', math.random(2, 4))
end)
