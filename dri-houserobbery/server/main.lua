...
DRICore.Functions.CreateCallback('dri-houserobbery:server:GetHouseConfig', function(source, cb)
    cb(Config.Houses)
end)

RegisterServerEvent('dri-houserobbery:server:enterHouse')
AddEventHandler('dri-houserobbery:server:enterHouse', function(house)
    local src = source
    ...
    if not Config.Houses[house]["opened"] then
        ...
        TriggerClientEvent('dri-houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('dri-houserobbery:client:enterHouse', src, house)
...
function ResetHouseStateTimer(house)
    ...
    Citizen.SetTimeout(time, function()
        ...
        TriggerClientEvent('dri-houserobbery:client:ResetHouseState', -1, house)
    end)
end

RegisterServerEvent('dri-houserobbery:server:searchCabin')
AddEventHandler('dri-houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    local Player = DRICore.Functions.GetPlayer(src)
    ...
    if itemFound < 4 then
        ...
        for i = 1, itemCount, 1 do
            ...
            local itemInfo = DRICore.Shared.Items[randomItem]
            if math.random(1, 100) == 69 then
                randomItem = "painkillers"
                itemInfo = DRICore.Shared.Items[randomItem]
        ...
        TriggerClientEvent('DRICore:Notify', src, 'The box is empty', 'error', 3500)
    end

    Config.Houses[house]["furniture"][cabin]["searched"] = true
    TriggerClientEvent('dri-houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterServerEvent('dri-houserobbery:server:SetBusyState')
AddEventHandler('dri-houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('dri-houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)
