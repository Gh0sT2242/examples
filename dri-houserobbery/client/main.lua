...
RegisterNetEvent('DRICore:Client:OnPlayerLoaded', function()
    DRICore.Functions.TriggerCallback('dri-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
CreateThread(function()
    Wait(500)
    requiredItems = {
        [1] = {name = DRICore.Shared.Items["lockpick"]["name"], image = DRICore.Shared.Items["lockpick"]["image"]},
        [2] = {name = DRICore.Shared.Items["screwdriverset"]["name"], image = DRICore.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['dri-interior']:CreateHouseRobbery(coords)
    end
    Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
    ...
    currentHouse = house
    Wait(500)
    TriggerEvent('dri-weathersync:client:DisableSync')
end
...
function leaveRobberyHouse(house)
    ...
    DoScreenFadeOut(250)
    Wait(500)
    exports['dri-interior']:DespawnInterior(houseObj, function()
        TriggerEvent('dri-weathersync:client:EnableSync')
        Wait(250)
        DoScreenFadeIn(250)
...
RegisterNetEvent('dri-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('dri-houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        PoliceCall()
                        TriggerEvent('dri-lockpick:client:openLockpick', lockpickFinish)
                        if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
                            local pos = GetEntityCoords(PlayerPedId())
                            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
                        end
                    else
                        DRICore.Functions.Notify('The door is already open..', 'error', 3500)
                    end
                else
                    DRICore.Functions.Notify('Not enough Police..', 'error', 3500)
                end
            end
        else
            DRICore.Functions.TriggerCallback('DRICore:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        if CurrentCops >= Config.MinimumHouseRobberyPolice then
                            if not Config.Houses[closestHouse]["opened"] then
                        ...
                        DRICore.Functions.Notify('It looks like you are missing something...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        ...
        if closestPed ~= nil then
            ...
            local gender = "Man"
            if DRICore.Functions.GetPlayerData().charinfo.gender == 1 then
                gender = "Woman"
            end
...
function GetNearbyPed()
	...
	local closestPed, closestDistance = DRICore.Functions.GetClosestPed(coords, PlayerPeds)
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('dri-houserobbery:server:enterHouse', closestHouse)
        DRICore.Functions.Notify('It worked!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = DRICore.Shared.Items["advancedlockpick"]
            if math.random(1, 100) < 20 then
                TriggerServerEvent("DRICore:Server:RemoveItem", "advancedlockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        else
            local itemInfo = DRICore.Shared.Items["lockpick"]
            if math.random(1, 100) < 40 then
                TriggerServerEvent("DRICore:Server:RemoveItem", "lockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        end
        
        DRICore.Functions.Notify('It did not work..', 'error', 2500)
    end
end

RegisterNetEvent('dri-houserobbery:client:setHouseState', function(house, state)
...
function searchCabin(cabin)
    ...
    local Skillbar = exports['dri-skillbar']:GetSkillbarObject()
    if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
    end
    LockpickDoorAnim(lockpickTime)
    TriggerServerEvent('dri-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('dri-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('dri-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('dri-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('dri-houserobbery:client:SetBusyState', function(cabin, house, bool)
