...
RegisterNetEvent("DRICore:Client:OnPlayerUnload")
AddEventHandler("DRICore:Client:OnPlayerUnload", function()
    isLoggedIn = false
end)
...
function openBindingMenu()
    local PlayerData = DRICore.Functions.GetPlayerData()
...
RegisterNetEvent('dri-commandbinding:client:openUI')
AddEventHandler('dri-commandbinding:client:openUI', function()
    openBindingMenu()
end)
        ...
        if isLoggedIn then
            for k, v in pairs(availableKeys) do
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    local keyMeta = DRICore.Functions.GetPlayerData().metadata["commandbinds"]
                    local args = {}
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            TriggerServerEvent('DRICore:CallCommand', keyMeta[v]["command"], args)
                            keyPressed = true
                        else
                            DRICore.Functions.Notify('There is still nothing ['..v..'] tied, /binds to bind a command', 'primary', 4000)
                        end
                    else
                        DRICore.Functions.Notify('You have not yet tied any commands, /binds to bind a command', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    DRICore.Functions.Notify('Command bindings are stored!', 'success')

    TriggerServerEvent('dri-commandbinding:server:setKeyMeta', keyData)
end)
